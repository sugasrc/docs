...
<Step title="Expose your app on HTTPS">
  ...
  **Public Network** section, click **Add Endpoint → HTTPS Domain** and choose
  ...
  <Step title="Wire the env var references">
    ...
    - `WEBHOOK_URL`: set to `https://{{n8n.variables.SUGA_PUBLIC_HOSTNAME}}/`. The `{{...}}` picker inserts the reference alongside literal text.
    - `N8N_PROTOCOL`: set to the literal `https`.

    The canvas now has both containers with their volumes attached:

    <Frame>
      <img src="/images/tutorials/deploy-docker-compose/n8n-canvas.png" alt="Suga canvas showing postgres and n8n containers with 2 GB volumes mounted at /var/lib/postgresql and /home/node/.n8n" />
    </Frame>
