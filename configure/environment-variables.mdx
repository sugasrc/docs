...
You can unmark a variable while you are still editing. Once you save, the choice is fixed.
...
Every service receives a set of variables describing where it can be reached. You will mostly use them through references, so one service can read another's address.
