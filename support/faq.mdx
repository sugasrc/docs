...
Two things are worth knowing before you plan around it. Free services can run Suga's approved template images or an image built from your own GitHub repository, but not an arbitrary image from a registry. Free services are also limited to 5 Mbit/s of outbound bandwidth and a 15 second limit per public request, which together cap a single response at roughly 9 MB. Both lift on a paid plan. See [Plan Limits](/reference/limits) for the full picture.
