...
Suga runs production workloads, and the reliability features are in place: zero-downtime deploys, safe rollouts that keep your running version live if a deploy fails, sub-10-second deployments, self-healing failover, and one-click rollback. We're still in early access and shipping quickly, so some features are in development and interfaces may change, and we give advance notice before any breaking change. Live status is at [status.suga.app](https://status.suga.app).
