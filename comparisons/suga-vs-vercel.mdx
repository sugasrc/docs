...
| Container support | 37+ frameworks auto-detected, no Dockerfile needed | Dockerfile, auto-detected source builds, or pre-built images from any registry |
| Cold starts | Effectively eliminated | None, since containers are always on |
| Billing | Active CPU time (I/O wait excluded) | Per-second metered across CPU, memory, storage, and egress |

Vercel's Fluid Compute model bills only for active CPU time, which is a meaningful advantage for I/O-heavy workloads like model inference or third-party API fan-out where the CPU spends a lot of time waiting on external services. Suga bills the allocated container per second across CPU, memory, storage, and egress, which is more predictable for always-on services where the CPU stays busy most of the time, so the right pricing model depends on whether your workload sits idle a lot or works the whole time.
...
| Bot protection | Managed rulesets + BotID | Cloudflare Bot Management at the zone level |
| Rate limiting | `@vercel/firewall` SDK | Cloudflare rate limiting at the zone level |
...
| Network isolation | Secure Compute with VPC peering (Enterprise) | Kubernetes NetworkPolicy with default-deny per namespace |

Vercel ships WAF, BotID, and rate limiting as platform primitives that you configure directly in the Vercel dashboard. Suga's edge security comes from the Cloudflare proxy in front of your app, where L3/L4/L7 DDoS protection and TLS termination apply automatically, while WAF, bot management, and rate limiting are configured at Suga's managed Cloudflare zone rather than as per-tenant settings you control. At the cluster layer, every Suga environment runs in its own Kubernetes namespace with default-deny network policies, which blocks lateral movement between environments even if a workload is compromised.
...
| Bot protection | Managed rulesets + BotID | Cloudflare Bot Management at the zone level |
| Rate limiting | All plans + SDK | Cloudflare rate limiting at the zone level |
...
| Databases | Managed providers via Marketplace | Container-based templates (Postgres, MySQL, Redis, Mongo, MinIO) |
| Private networking | Secure Compute with VPC peering (Enterprise) | Kubernetes namespace isolation with NetworkPolicy |
...
- [Talk to us](../support/contact.mdx) if you're evaluating a migration from Vercel
