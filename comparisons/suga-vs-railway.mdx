...
Suga runs on GKE Autopilot behind Cloudflare's global edge by default, meters usage as you go, and bundles $20 of hosting credits into every Pro seat before usage charges accrue. On the Enterprise plan, Suga can also drive your own Kubernetes cluster across any cloud or on-premises, so the same control plane handles Suga's managed cluster, your GKE cluster, your EKS cluster, or an on-prem cluster.
