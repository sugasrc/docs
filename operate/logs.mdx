...
<Note>
  Individual JSON keys are not broken out into separate searchable fields. Search is a case-insensitive substring match over the whole message, so searching for `user_id` finds lines containing that text anywhere. Filtering by level, service, and replica is done with the facets, not the search box.
</Note>
