...
Two limits apply to Free services on Suga Cloud and to no other tier.

| Limit | Free | Pro and Enterprise |
| ----- | ---- | ------------------ |
| Outbound bandwidth | 5 Mbit/s per replica | Unlimited |
| Public request duration | 15 seconds | Unlimited |

These two combine into a practical response size ceiling. At 5 Mbit/s a service can send about 625 KB per second, so a response still sending after 15 seconds is cut off mid-flight. That puts the largest deliverable response at roughly 9 MB, and lower if the service spends any of those 15 seconds thinking before it starts sending.
