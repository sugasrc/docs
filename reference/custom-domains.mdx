...
Custom domains let you serve your Suga services from your own domain (e.g., `app.yourdomain.com`) instead of the auto-generated Suga domains.
...
<Steps>
  ...
  <Step title="Add a custom domain">
    In the **Public Networking** section, click **+ Custom Domain**. Enter your domain name (e.g., `app.yourdomain.com`) and the target port your application listens on (e.g., `3000`).
  </Step>
...
Apex (root) domains like `yourdomain.com` (without a subdomain prefix) require special DNS support because the DNS specification doesn't allow CNAME records at the zone apex.
...
<Note>
  If your DNS provider does not support CNAME flattening, use a subdomain like `www.yourdomain.com` instead and set up a redirect from the apex domain to it.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Do I need to manage TLS certificates?">
    No. Suga automatically provisions and renews TLS certificates for all custom domains.
  </Accordion>
  ...
  <Accordion title="Why can't I use an apex domain with my DNS provider?">
    Standard DNS does not allow CNAME records at the zone apex. Your provider needs to support CNAME flattening, ANAME, or ALIAS records. If it doesn't, use a subdomain like `www.yourdomain.com` instead.
  </Accordion>
</AccordionGroup>
